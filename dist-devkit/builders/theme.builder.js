...
function commandBuilder(options, context) {
    return __awaiter(this, void 0, void 0, function () {
        var outPath, prjMetadata, pkgFile, pkgFilePath;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    outPath = Path.join(context.workspaceRoot, options.outputPath);
                    // Remove the dist folder and recreate it fresh
                    FS.removeSync(outPath);
                    FS.ensureDir(outPath);
                    return [4 /*yield*/, context.getProjectMetadata(context.target)];
                case 1:
                    prjMetadata = _a.sent();
                    // Copy all files from root to the outPath
                    FS.copySync(prjMetadata.root, outPath);
                    pkgFile = packages_1.Packages.loadPackageFile(outPath);
                    if (pkgFile !== null) {
                        delete pkgFile.scripts;
                        pkgFilePath = Path.join(outPath, 'package.json');
